...
class JuheAPICollectionViewCell:BaseCollectionViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:0, y:YH(obj: self.icon), width:self.icon.bounds.size.width, height: 20))
...
view.backgroundColor = .white
    ...
    layout.itemSize = CGSize(width: (APPW-50)/4, height:90);
        layout.sectionInset = UIEdgeInsets(top: YH(obj: banner)+10, left: 10, bottom: 0, right:10)
    ...
    self.collectionView.delegate = self;
        self.collectionView.register(JuheAPICollectionViewCell.self, forCellWithReuseIdentifier: JuheAPICollectionViewCell.getColloctionCellIdentifier())
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: JuheAPICollectionViewCell.getColloctionCellIdentifier(), for: indexPath) as? JuheAPICollectionViewCell
