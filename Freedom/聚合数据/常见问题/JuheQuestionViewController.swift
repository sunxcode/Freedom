...
class JuheQuestionViewCell:BaseTableViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:XW(obj: self.icon)+10, y: 10, width: APPW-XW(obj: self.icon)-20, height: 20))
...
class JuheQuestionViewController: JuheBaseViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: JuheQuestionViewCell.getTableCellIdentifier()) as? JuheQuestionViewCell
        if cell == nil{
