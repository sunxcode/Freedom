...
class JuheDynamicCollectionViewCell:BaseCollectionViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:10, y:YH(obj: self.icon), width:self.bounds.size.width-20, height: 20))
...
self.collectionView.register(JuheDynamicCollectionViewCell.self, forCellWithReuseIdentifier: JuheDynamicCollectionViewCell.getColloctionCellIdentifier())
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: JuheDynamicCollectionViewCell.getColloctionCellIdentifier(), for: indexPath) as? JuheDynamicCollectionViewCell
