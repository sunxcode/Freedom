...
class JuheUserViewCell:BaseTableViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:XW(obj: self.icon)+10, y:20, width:200, height: 20))
...
class JuheUserViewController: JuheBaseViewController {
    ...
    let name = UILabel(frame: CGRect(x: XW(obj: icon)+10, y: 10, width: 300, height: 20))
    name.text = "用户名：18326891683  👑已认证"
    let openid = UILabel(frame: CGRect(x: name.frame.origin.x, y:YH(obj: name), width: 400, height: 20))
        openid.text = "OpenId:JH12bd23ef316e3d8a9dfe7402ef8bc453"
    let email = UILabel(frame: CGRect(x: name.frame.origin.x, y: YH(obj: openid), width: 300, height: 20))
    email.text = "绑定邮箱:1069106050@qq.com"
    let phone = UILabel(frame: CGRect(x: name.frame.origin.x, y: YH(obj: email), width: 300, height: 20))
