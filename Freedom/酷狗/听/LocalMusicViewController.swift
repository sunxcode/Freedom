...
import MediaToolbox
import MediaPlayer
class LocalMusicCell: BaseTableViewCell {
...
class LocalMusicViewController: KugouBaseViewController {
    ...
    func setupTableView() {
        let tableView = UITableView(frame: CGRect(x: 0, y: 64, width: APPW, height: APPH), style: .plain)
        tableView.delegate = self
        tableView.dataSource = self
        self.tableView = tableView as! BaseTableView
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell :LocalMusicCell = LocalMusicCell.getInstance() as! LocalMusicCell
