...
+ (NSData *)gtm_dataByCompressingBytes:(const void *)bytes
    ...
    if ((retCode = deflateInit2(&strm, level, Z_DEFLATED, windowBits,
        ...
        DLog(@"Failed to init for deflate w/ level %d, error %d",
                   level, retCode);
        return nil;
        // COV_NF_END
    }
...
+ (NSData *)gtm_dataByInflatingBytes:(const void *)bytes
    ...
    do {
        ...
        if ((retCode != Z_OK) && (retCode != Z_STREAM_END)) {
            DLog(@"Error trying to inflate some of the payload, error %d",
                       retCode);
            inflateEnd(&strm);
            return nil;
        }
    ...
    // stream.
    if (strm.avail_in != 0) {
        DLog(@"thought we finished inflate w/o using all input, %u bytes left",
                   strm.avail_in);
        result = nil;
    }
