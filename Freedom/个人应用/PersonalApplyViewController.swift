...
class PersonalApplyViewCell:BaseTableViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:XW(obj: self.icon)+10, y: 20, width: APPW-XW(obj: self.icon)-20, height: 20))
        self.title.font = UIFont.systemFont(ofSize: 14)
        self.title.textAlignment = .center
        self.script = UILabel(frame:CGRect(x: self.title.x(), y: YH(obj: self.title), width: self.title.width(), height: self.title.height()))
...
class PersonalApplyViewController: PersonalApplyBaseViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: PersonalApplyViewCell.getTableCellIdentifier()) as? PersonalApplyViewCell
        if cell == nil{
