...
class EnergyShopViewCell:BaseTableViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x:XW(obj: self.icon)+20, y:(80-20)/2.0, width: APPW-XW(obj: self.icon), height: 20))
...
class EnergyShopViewController: EnergyBaseViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: EnergyShopViewCell.getTableCellIdentifier()) as? EnergyShopViewCell
        if cell == nil{
