...
class EnergyHomeViewCell:BaseCollectionViewCell{
    override func initUI() {
        ...
        self.title = UILabel(frame: CGRect(x: 0, y: YH(obj: self.icon), width:self.icon.width(), height: 20))
...
self.collectionView.register(EnergyHomeViewCell.self, forCellWithReuseIdentifier: EnergyHomeViewCell.getColloctionCellIdentifier())
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: EnergyHomeViewCell.getColloctionCellIdentifier(), for: indexPath) as? EnergyHomeViewCell
        return cell!
    }
