...
@end
@implementation BaseScrollOCView
...
//FIXME:分类平分展示视图
+(BaseScrollOCView *)sharedSegmentWithFrame:(CGRect)frame Titles:(NSArray*)titles{
    BaseScrollOCView *segment = [[self alloc]initWithFrame:frame];
...
//FIXME:标题滑动，自适应文字宽度
+(BaseScrollOCView *)sharedTitleScrollWithFrame:(CGRect)frame Titles:(NSArray*)titles{
    BaseScrollOCView *titleScroll = [[self alloc]initWithFrame:frame];
...
//FIXME:标题滑动，有文字和图标的
+(BaseScrollOCView *)sharedTitleIconScrollWithFrame:(CGRect)frame Titles:(NSArray *)titles icons:(NSArray *)icons{
    BaseScrollOCView *titleScroll = [[self alloc]initWithFrame:frame];
...
//FIXME:内容小项目滑动，一般在主页
+(BaseScrollOCView *)sharedBaseItemWithFrame:(CGRect)frame icons:(NSArray*)icons titles:(NSArray*)titles size:(CGSize)size round:(BOOL)round{
    BaseScrollOCView *baseItem = [[self alloc]initWithFrame:frame];
...
+(BaseScrollOCView *)sharedScrollItemWithFrame:(CGRect)frame icons:(NSArray *)icons titles:(NSArray *)titles size:(CGSize)size hang:(NSInteger)hang round:(BOOL)round{
    BaseScrollOCView *baseItem = [[self alloc]initWithFrame:frame];
...
//FIXME:内容视图滑动，如新闻类
+(BaseScrollOCView *)sharedContentViewWithFrame:(CGRect)frame controllers:(NSArray*)controllers inViewController:(UIViewController *)NVC{
    BaseScrollOCView *contentView = [[self alloc]initWithFrame:frame];
...
+(BaseScrollOCView *)sharedContentTitleViewWithFrame:(CGRect)frame titles:(NSArray *)titles controllers:(NSArray *)controllers inViewController:(UIViewController *)NVC{
    BaseScrollOCView *contentView = [[self alloc]initWithFrame:frame];
    contentView.tag = MyScrollTypeContentTitleView;
    contentView.titleScrollView = [BaseScrollOCView sharedSegmentWithFrame:CGRectMake(frame.origin.x, frame.origin.y, W(contentView), 40) Titles:titles];
    contentView.contentScrollView = [BaseScrollOCView sharedContentViewWithFrame:CGRectMake(X(contentView),YH(contentView.titleScrollView), W(contentView), H(contentView)-H(contentView.titleScrollView)) controllers:controllers inViewController:NVC];
    contentView.pushDelegateVC = NVC;
    __weak __typeof(BaseScrollOCView*)weakSelf = contentView;
...
+(BaseScrollOCView *)sharedContentIconViewWithFrame:(CGRect)frame titles:(NSArray *)titles icons:(NSArray *)icons controllers:(NSArray *)controllers inViewController:(UIViewController *)NVC{
    BaseScrollOCView *contentView = [[self alloc]initWithFrame:frame];
    contentView.tag = MyScrollTypeContentIconView;
    contentView.titleScrollView = [BaseScrollOCView sharedTitleIconScrollWithFrame:CGRectMake(frame.origin.x, frame.origin.y, W(contentView), 60)  Titles:titles icons:icons];
    contentView.contentScrollView = [BaseScrollOCView sharedContentViewWithFrame:CGRectMake(X(contentView),YH(contentView.titleScrollView), W(contentView), H(contentView)-H(contentView.titleScrollView)) controllers:controllers inViewController:NVC];
    contentView.pushDelegateVC = NVC;
    __weak __typeof(BaseScrollOCView*)weakSelf = contentView;
    contentView.titleScrollView.selectBlock = ^(NSInteger index, NSDictionary *dict) {
...
+(BaseScrollOCView *)sharedBannerWithFrame:(CGRect)frame icons:(NSArray*)icons{
    BaseScrollOCView *banner = [[self alloc]initWithFrame:frame];
...
//FIXME:竖着动态播放的视图或广告
+(BaseScrollOCView *)sharedVerticallyBannerWithFrame:(CGRect)frame icons:(NSArray*)icons{
    BaseScrollOCView *banner = [[self alloc]initWithFrame:frame];
    [banner setBannerWithicons:icons];
    return banner;
}
...
//FIXME:欢迎界面
+(BaseScrollOCView *)sharedWelcomWithFrame:(CGRect)frame icons:(NSArray*)icons{
    BaseScrollOCView *welcom = [[self alloc]initWithFrame:frame];
...
#pragma mark 另加的
//FIXME:分类展示的标题栏
+(BaseScrollOCView *)sharedViewSegmentWithFrame:(CGRect)frame viewsNumber:(NSInteger)num viewOfIndex:(viewOfIndexBlock)block{
    BaseScrollOCView *contentView = [[BaseScrollOCView alloc]initWithFrame:frame];
...
//FIXME:内容小项目视图
+(BaseScrollOCView *)sharedViewItemWithFrame:(CGRect)frame viewsNumber:(NSInteger)num viewOfIndex:(viewOfIndexBlock)block{
    BaseScrollOCView *contentView = [[BaseScrollOCView alloc]initWithFrame:frame];
    contentView.viewBlock = block;
...
+(BaseScrollOCView *)sharedViewBannerWithFrame:(CGRect)frame viewsNumber:(NSInteger)num viewOfIndex:(viewOfIndexBlock)block Vertically:(BOOL)vertical setFire:(BOOL)fire{
    BaseScrollOCView *contentView = [[BaseScrollOCView alloc]initWithFrame:frame];
    contentView.viewBlock = block;
...
//FIXME:集合配置
+(BaseScrollOCView *)sharedWithFrame:(CGRect)frame Titles:(NSArray *)titles icons:(NSArray *)icons round:(BOOL)round size:(CGSize)size type:(MyScrolltype)type controllers:(NSArray*)controllers selectIndex:(selectIndexBlock)block{
    BaseScrollOCView *ref = [[self alloc]initWithFrame:frame];
