...
-(HTMLNodeType)nodetype{
    if (_node == NULL || _node->name == NULL)return HTMLUnkownNode;
    const char * tagName = (const char*)_node->name;
    if (strcmp(tagName, "a") == 0)return HTMLHrefNode;
    else if (strcmp(tagName, "text") == 0)return HTMLTextNode;
    else if (strcmp(tagName, "code") == 0)return HTMLCodeNode;
    else if (strcmp(tagName, "span") == 0)return HTMLSpanNode;
    else if (strcmp(tagName, "p") == 0)return HTMLPNode;
    else if (strcmp(tagName, "ul") == 0)return HTMLUlNode;
    else if (strcmp(tagName, "li") == 0)return HTMLLiNode;
    else if (strcmp(tagName, "image") == 0)return HTMLImageNode;
    else if (strcmp(tagName, "ol") == 0)return HTMLOlNode;
    else if (strcmp(tagName, "strong") == 0)return HTMLStrongNode;
    else if (strcmp(tagName, "pre") == 0)return HTMLPreNode;
    else if (strcmp(tagName, "blockquote") == 0)return HTMLBlockQuoteNode;
    else return HTMLUnkownNode;
}
...
-(NSString*)rawContents {
    ...
    htmlNodeDumpOutput(buf, _node->doc, _node, (const char*)_node->doc->encoding);
...
-(id)initWithData:(NSData*)data error:(NSError**)error{
	if (self = [super init]){
		...
		if (data){
			...
			_doc = htmlReadDoc((xmlChar*)[data bytes],"",enc,
