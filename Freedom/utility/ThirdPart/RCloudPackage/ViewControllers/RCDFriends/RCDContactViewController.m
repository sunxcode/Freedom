...
- (UISearchBar *)searchFriendsBar {
    if (!_searchFriendsBar) {
        _searchFriendsBar=[[UISearchBar alloc]initWithFrame:CGRectMake(2, 0, APPW-4, 28)];
...
- (UITableView *)friendsTabelView {
    if (!_friendsTabelView) {
        _friendsTabelView=[[UITableView alloc]initWithFrame:CGRectMake(0.0, 43.5, APPW, APPH-43.5-114) style:UITableViewStyleGrouped];
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self sortAndRefreshWithList:[self getAllFriendList]];
    UIButton *buttonItem = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 18, 20)];
    UIImageView *imageV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"add_friend"]];
    imageV.frame = CGRectMake(0, 0, 18, 20);
    [buttonItem addSubview:imageV];
    [buttonItem addTarget:self action:@selector(pushAddFriend:) forControlEvents:UIControlEventTouchUpInside];
    NSArray<UIBarButtonItem *> *barButtonItems;
    UIBarButtonItem *negativeSpacer = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
    negativeSpacer.width = -6;
    barButtonItems = [NSArray arrayWithObjects:negativeSpacer, [[UIBarButtonItem alloc]initWithCustomView:buttonItem], nil];
 ...
 numberOfRowsInSection:(NSInteger)section {
  ...
  if (section == 0) {
    ...
    NSString *letter = self.resultDic[@"allKeys"][section -1];
      NSArray *temp = self.allFriendSectionDic[letter];
    rows = [temp count];
  }
  return rows;
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    NSArray *temp = self.resultDic[@"allKeys"];
  return [temp count] + 1;
}
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
  dispatch_async(dispatch_get_global_queue(0, 0), ^{
    self.resultDic = [FreedomTools sortedArrayWithPinYinDic:friendList];
