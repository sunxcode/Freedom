...
- (void)viewDidLoad {
  ...
  if (portraitUri.length == 0) {
    ...
    [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
      ...
      NSString *firstLetter = [ChineseToPinyin firstPinyinFromChinise:self.friendInfo.name];
...
- (void)btnConversation:(id)sender {
    [RCDMainTabBarViewController shareInstance].selectedIndex = 0;
  //创建会话
    ...
    case 0: {
      ...
      if (!self.inBlackList) {
        ...
        [[RCIMClient sharedRCIMClient]
         addToBlacklist:self.friendInfo.userId
         success:^{
           weakSelf.inBlackList = YES;
        ...
        [[RCIMClient sharedRCIMClient]
         removeFromBlacklist:self.friendInfo.userId
         success:^{
           dispatch_async(dispatch_get_main_queue(), ^{
             [hud hide:YES];
           });
