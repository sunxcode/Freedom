...
- (void)viewDidLoad {
  ...
  verificationCodeField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"验证码" attributes:@{NSForegroundColorAttributeName : color}];
  ...
  [sendCodeButton setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f green:133 / 255.f blue:133 / 255.f alpha:1]];
  ...
  //      [[UIColor alloc] initWithRed:153 green:153 blue:153 alpha:0.5];
  [vCodeTimerLb setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f green:133 / 255.f blue:133 / 255.f alpha:1]];
  vCodeTimerLb.textColor = [UIColor whiteColor];
  vCodeTimerLb.textAlignment = NSTextAlignmentCenter;
  ...
  passwordTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"新密码" attributes:@{NSForegroundColorAttributeName : color}];
  ...
  UIButton *registerButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.bounds.size.width - 100, -16, 80, 50)];
  ...
  [self.view addConstraint:[NSLayoutConstraint
                               constraintWithItem:_rongLogo
                                       ...
                                       multiplier:1.0
                                         constant:0]];
  NSDictionary *views = NSDictionaryOfVariableBindings(_errorMsgLb, _licenseLb, _rongLogo, _inputBackground, bottomBackground,vCodeTimerLb);
  NSArray *viewConstraints = [[[[[[[NSLayoutConstraint
      ...
      arrayByAddingObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:@"V:|-70-[_rongLogo(100)]-[_"
                                          @"errorMsgLb(==15)]-[_"
                                          @"inputBackground("
                                          @"==235)]"
                                  options:0
  ...
  NSDictionary *inputViews = NSDictionaryOfVariableBindings(verificationCodeField, pswMsgLb, userNameTextField, passwordTextField,loginButton, vCodeTimerLb, sendCodeButton, vCodeTimerLb);
      ...
      arrayByAddingObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:
                  @"H:|[verificationCodeField]-[sendCodeButton(80)]|"
                                  options:0
                                  metrics:nil
                                    views:inputViews]]
      arrayByAddingObjectsFromArray:
          [NSLayoutConstraint constraintsWithVisualFormat:
                                  @"H:|[verificationCodeField]-[vCodeTimerLb]|"
                                                  options:0
                                                  metrics:nil
                                                    views:inputViews]]
      arrayByAddingObjectsFromArray:
...
- (void)textFieldDidChange:(UITextField *)textField {
  if (textField.tag == UserTextFieldTag) {
    if (textField.text.length > 0) {
      ...
      [((UIButton *)[self.view viewWithTag:SendCodeButtonTag]) setBackgroundColor:[[UIColor alloc] initWithRed:23 / 255.f green:136 / 255.f blue:213 / 255.f alpha:1]];
    }
    if (textField.text.length == 0) {
            ...
            if ([response[@"result"] integerValue] == 0) {
              [AFHttpTool getVerificationCode:@"86"
                  ...
                  success:^(id response) {
                    [hud hide:YES];
                    if ([response[@"code"] intValue] == 200) {
                      ((UILabel *)[self.view viewWithTag:vCodeTimerLabelTag]).hidden = NO;
                      ((UIButton *)[self.view viewWithTag:SendCodeButtonTag]).hidden = YES;
                      [self CountDown:60];
                    }
                  }failure:^(NSError *err) {
                    NSLog(@"%@", err);
...
- (IBAction)btnDoneClicked:(id)sender {
  ...
  [AFHttpTool verifyVerificationCode:@"86" phoneNumber:phone verificationCode:vCode success:^(id response) {
        if ([response[@"code"] intValue] == 200) {
          NSDictionary *result = response[@"result"];
          NSString *vToken = result[@"verification_token"];
          [AFHttpTool resetPassword:userPwd vToken:vToken success:^(id response) {
                if ([response[@"code"] intValue] == 200) {
