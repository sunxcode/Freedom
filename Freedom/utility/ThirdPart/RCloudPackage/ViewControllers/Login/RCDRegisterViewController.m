...
- (void)viewDidLoad {
  ...
  verificationCodeField.attributedPlaceholder = [[NSAttributedString alloc]initWithString:@"短信验证码" attributes:@{NSForegroundColorAttributeName : color}];
  ...
  [verificationCodeField addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
  ...
  [_getVerificationCodeBt setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f green:133 / 255.f blue:133 / 255.f alpha:1]];
  ...
  [_getVerificationCodeBt addTarget:self action:@selector(getVerficationCode) forControlEvents:UIControlEventTouchUpInside];
  ...
  [_countDownLable setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f green:133 / 255.f blue:133 / 255.f alpha:1]];
  ...
  rePasswordTextField.attributedPlaceholder = [[NSAttributedString alloc]initWithString:@"昵 称" attributes:@{NSForegroundColorAttributeName : color}];
  ...
  NSDictionary *inputViews = NSDictionaryOfVariableBindings(userNameMsgLb, pswMsgLb, userNameTextField, passwordTextField,loginButton, rePasswordTextField, verificationCodeField,verificationCodeLb, _getVerificationCodeBt);
...
- (void)textFieldDidChange:(UITextField *)textField {
  if (textField.tag == UserTextFieldTag) {
    if (textField.text.length > 0) {
      ...
      [_getVerificationCodeBt setBackgroundColor:[[UIColor alloc] initWithRed:23 / 255.f green:136 / 255.f blue:213 / 255.f alpha:1]];
    }
    if (textField.text.length == 0) {
      _getVerificationCodeBt.enabled = NO;
      [_getVerificationCodeBt setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f green:133 / 255.f blue:133 / 255.f alpha:1]];
...
- (void)getVerficationCode {
  ...
  if (_PhoneNumber.length == 11) {
    ...
    [AFHttpTool checkPhoneNumberAvailable:@"86" phoneNumber:phone success:^(id response) {
          if ([response[@"code"] intValue] == 200) {
            if ([response[@"result"] integerValue] == 0) {
              ...
              [AFHttpTool getVerificationCode:@"86" phoneNumber:phone success:^(id response) {
                    [hud hide:YES];
...
- (IBAction)btnDoneClicked:(id)sender {
  ...
  //验证验证码是否有效
  [AFHttpTool verifyVerificationCode:@"86" phoneNumber:userName verificationCode:verificationCode success:^(id response) {
        ...
        if (code.intValue == 200) {
          ...
          //注册用户
      [AFHttpTool registerWithNickname:nickName password:userPwd verficationToken:verificationToken success:^(id response) {
