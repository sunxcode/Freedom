...
#define RC_REAL_TIME_LOCATION_STATUS_FRAME                                     \
CGRectMake(0, 62, self.frame.size.width, 38)
#define RC_REAL_TIME_LOCATION_EXPEND_FRAME                                     \
CGRectMake(0, 62, self.frame.size.width, 85)
...
- (UILabel *)expendLabel {
    if (!_expendLabel) {
        _expendLabel = [[UILabel alloc]
                        initWithFrame:CGRectMake(30, 0, self.frame.size.width - 48, 60)];
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton
         setBackgroundImage:[UIImage imageNamed:@"location_share_button"]
         forState:UIControlStateNormal];
        [_cancelButton
         setBackgroundImage:[UIImage imageNamed:@"location_share_button_hover"]
...
- (UIButton *)joinButton {
    if (!_joinButton) {
        _joinButton = [[UIButton alloc]
                       initWithFrame:CGRectMake(self.frame.size.width - 50 - 79, 52, 50, 25)];
...
- (void)setDataModel:(RCMessageModel *)model {
    ...
    self.tipMessageLabel.frame = CGRectMake(
  ...
  dispatch_async(dispatch_get_global_queue(0, 0), ^{
    ...
    for (RCUserInfo *user in groupList) {
      ...
      if ([user.portraitUri hasPrefix:@"file:///"]) {
        NSString *filePath = [FreedomTools
