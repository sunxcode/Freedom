...
- (BOOL)handleWatchKitRequest {
  ...
  if ([query isEqualToString:WK_APP_COMMUNICATE_QUERY_UNREAD_COUNT]) {
    ...
    NSData *data =
        [NSKeyedArchiver archivedDataWithRootObject:conversationList];
    ...
    [self replyWKApp:data];
                        ...
                        WK_APP_COMMUNICATE_REQUEST_CACHE_ALL_HEAD_ICON]) {
        ...
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          if (![fileManager fileExistsAtPath:[currentUrl path]]) {
            if (conversationType.intValue == ConversationType_PUBLICSERVICE ||
              ...
              NSArray *publicServices =
                  [[RCIMClient sharedRCIMClient] getPublicServiceList];
    ...
    if ([RCIMClient sharedRCIMClient].watchKitStatusDelegate == nil) {
      [RCIMClient sharedRCIMClient].watchKitStatusDelegate =
          [RCWKNotifier sharedWKNotifier];
    }
    [self replyWKApp:nil];
    ...
    if ([self isLocalPath:imageUrl]) {
      ...
      [[RCIMClient sharedRCIMClient]
          downloadMediaFile:[conversationType intValue]
          targetId:targetId
...
- (UIImage *)scaleImage:(UIImage *)image
  ...
  if (round) {
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(maxWidth, maxWidth), NO,
                                           1.0);
    [[UIBezierPath bezierPathWithRoundedRect:frame cornerRadius:maxWidth / 2]
        addClip];
