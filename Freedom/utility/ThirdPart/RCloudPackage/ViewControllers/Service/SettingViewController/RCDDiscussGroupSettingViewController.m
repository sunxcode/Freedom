...
- (void)viewDidLoad {
    ...
    [self.navigationController.navigationBar
     setBarTintColor:[UIColor colorWithRGBHex:0x0195ff]];
        ...
        success:^(RCDiscussion *discussion) {
          if (discussion) {
            ...
            for (NSString *targetId in discussion.memberIdList) {
              [RCDHTTPTOOL
                  getUserInfoByUserID:targetId
                           completion:^(RCUserInfo *user) {
                             if ([discussion.creatorId
                                     isEqualToString:user.userId]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  case 0: {
    RCDDiscussSettingCell *discussCell =
        [[RCDDiscussSettingCell alloc] initWithFrame:CGRectZero];
  ...
  case 1: {
    if (self.isOwner) {
      RCDDiscussSettingSwitchCell *switchCell =
          [[RCDDiscussSettingSwitchCell alloc] initWithFrame:CGRectZero];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (indexPath.row == 0) {
    ...
    UINavigationController *navi = [[UINavigationController alloc]
        initWithRootViewController:updateNameViewController];
