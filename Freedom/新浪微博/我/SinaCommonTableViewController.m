...
@end
@implementation SinaCommonLabelItem
@end
@implementation SinaCommonArrowItem
@end
@interface SinaCommonCell()
/*箭头*/
...
+ (instancetype)cellWithTableView:(UITableView *)tableView{
    ...
    SinaCommonCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[SinaCommonCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)setItem:(SinaMode *)item{
    ...
    } else if ([item isKindOfClass:[SinaCommonArrowItem class]]) {
        self.accessoryView = self.rightArrow;
    } else if ([item isKindOfClass:[SinaMode class]]) {
        self.accessoryView = self.rightSwitch;
    } else if ([item isKindOfClass:[SinaCommonLabelItem class]]) {
        SinaCommonLabelItem *labelItem = (SinaCommonLabelItem *)item;
        // 设置文字
...
@end
@implementation SinaCommonGroup
+ (instancetype)group{
    return [[self alloc] init];
}
@end
@interface SinaCommonTableViewController ()
@property (nonatomic, strong) NSMutableArray *groups;
@end
@implementation SinaCommonTableViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    SinaCommonGroup *group = self.groups[section];
    return group.items.count;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SinaCommonCell *cell = [SinaCommonCell cellWithTableView:tableView];
    SinaCommonGroup *group = self.groups[indexPath.section];
