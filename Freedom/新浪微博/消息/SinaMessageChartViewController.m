//  SinaMessageChartViewController.m
//  Created by Super on 16/8/21.
//  Copyright © 2016年 Super. All rights reserved.
//
#import "SinaMessageChartViewController.h"
@interface SinaMessageChartData : NSObject
@property(nonatomic,strong)NSString *content;
@property(nonatomic)BOOL fromMe;
+(NSArray*)getChartData;
@end
@implementation SinaMessageChartData
+(NSArray *)getChartData{
    SinaMessageChartData *m1=[[SinaMessageChartData alloc]init];
    m1.content=@"你好啊，美女！约吗？❤️";
    m1.fromMe=YES;
    SinaMessageChartData *m2=[[SinaMessageChartData alloc]init];
    m2.content=@"约在哪里？你家我家还是如家？今天明天还是七天？/害羞";
    m2.fromMe=NO;
    SinaMessageChartData *m3=[[SinaMessageChartData alloc]init];
    m3.content=@"那就来我家吧！👌！";
    m3.fromMe=YES;
    SinaMessageChartData *m4=[[SinaMessageChartData alloc]init];
    m4.content=@"好的！";
    m4.fromMe=NO;
    return @[m1,m2,m3,m4];
}
...
@end
@interface SinaMessageChartViewController ()<UITableViewDataSource,UITableViewDelegate>
@property (weak, nonatomic) IBOutlet UITextField *textField;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *bottomlayoutConstraint;
@property(nonatomic,strong)NSMutableArray *allMessage;
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@end
@implementation SinaMessageChartViewController
-(NSMutableArray *)allMessage{
    if(!_allMessage){
        _allMessage = [[SinaMessageChartData getChartData]mutableCopy];
    }return _allMessage;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (IBAction)clickReturnKey:(UITextField *)sender {//点击右下角返回键，发消息，手键盘。
    ...
    SinaMessageChartData *newMessage=[[SinaMessageChartData alloc]init];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SinaMessageChartData *mssg=self.allMessage[indexPath.row];
    //显示内容
