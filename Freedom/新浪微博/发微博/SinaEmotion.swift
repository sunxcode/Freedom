...
class SinaEmotionTextView: UITextView {
    ...
    override init(frame: CGRect, textContainer: NSTextContainer?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.textDidChange), name: .UITextViewTextDidChange, object: self)
    }
    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        placeholder.draw(in: placeholderRect, withAttributes:[NSAttributedStringKey.foregroundColor:UIColor.red])
    }
    ...
    func fullText() -> String {
        ...
        attributedText?.enumerateAttributes(in: NSRange(location: 0, length: attributedText?.length ?? 0), options: [], using: { attrs, range, stop in
            let attch = attrs[NSAttributedStringKey.attachment] as? SinaEmotionAttachment
            if attch != nil {
                // 图片
