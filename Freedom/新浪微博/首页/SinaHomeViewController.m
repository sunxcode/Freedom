...
//        DLog(@"请求失败");
    ...
    SinaLoadMoreFooter *load = [[SinaLoadMoreFooter alloc]init];
    load.hidden = YES;
...
-(NSArray *)stausFramesWithStatuses:(NSArray *)statuses{
    ...
    for (SinaStatus *status in statuses) {
        SinaStatusFrame *f = [[SinaStatusFrame alloc] init];
        f.status = status;
        [frames addObject:f];
    }
    return frames;
...
-(void)refreshStatus:(UIRefreshControl *)control {
    ...
    SinaStatusFrame *firstStatusF = [self.statusFrames firstObject];
    if (firstStatusF) {
...
-(void)titleClick:(UIButton *)titleButton{
    SinaDropdownView *dropMenu = [SinaDropdownView menu];
    dropMenu.delegate = self;
    SinaTitleMenuViewController *vc = [[SinaTitleMenuViewController alloc]init];
...
-(void)loadMoreStatus {
    ...
    SinaStatusFrame *lastStatusF = [self.statusFrames lastObject];
    if (lastStatusF) {
...
/*下拉菜单被销毁了*/
- (void)dropdownMenuDidDismiss:(SinaDropdownView *)menu{
    UIButton *titleButton = (UIButton *)self.navigationItem.titleView;
    titleButton.selected = NO;
}
/*下拉菜单显示了*/
- (void)dropdownMenuDidShow:(SinaDropdownView *)menu{
    UIButton *titleButton = (UIButton *)self.navigationItem.titleView;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SinaStatusViewCell *cell = [SinaStatusViewCell cellWithTableView:tableView];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    SinaStatusFrame *frame = self.statusFrames[indexPath.row];
