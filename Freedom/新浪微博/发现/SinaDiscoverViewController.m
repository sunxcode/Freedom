...
- (void)setupGroup0{
    ...
    // 2.设置组的基本数据
    // 3.设置组的所有行数据
    SinaCommonArrowItem *hotStatus = [SinaCommonArrowItem itemWithTitle:@"热门微博" icon:@"hot_status"];
    hotStatus.subtitle = @"笑话，娱乐，神最右都搬到这啦";
    SinaCommonArrowItem *findPeople = [SinaCommonArrowItem itemWithTitle:@"找人" icon:@"find_people"];
...
- (void)setupGroup1{
    ...
    gameCenter.destVcClass = [SinaDiscoverDetailViewController class];
    SinaCommonLabelItem *near = [SinaCommonLabelItem itemWithTitle:@"周边" icon:@"near"];
    near.text = @"测试文字";
    near.destVcClass = [SinaDiscoverDetailViewController class];
    SinaMode *app = [SinaMode itemWithTitle:@"应用" icon:@"app"];
    app.badgeValue = @"10";
    app.destVcClass = [SinaDiscoverDetailViewController class];
...
- (void)setupGroup2{
    ...
    SinaCommonLabelItem *cast = [SinaCommonLabelItem itemWithTitle:@"播客" icon:@"cast"];
    cast.operation = ^{
        DLog(@"----点击了播客");
    };
    cast.badgeValue = @"5";
    cast.subtitle = @"(10)";
    cast.text = @"axxxx";
    SinaCommonArrowItem *more = [SinaCommonArrowItem itemWithTitle:@"更多" icon:@"more"];
    //    more.badgeValue = @"998";
