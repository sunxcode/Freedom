...
+ (WXExpressionHelper *)sharedHelper{
    ...
    dispatch_once(&onceToken, ^{
        helper = [[WXExpressionHelper alloc] init];
    });
    return helper;
}
- (NSArray *)userEmojiGroups{
    return [self.store expressionGroupsByUid:[WXUserHelper sharedHelper].user.userID];
}
- (BOOL)addExpressionGroup:(TLEmojiGroup *)emojiGroup{
    BOOL ok = [self.store addExpressionGroup:emojiGroup forUid:[WXUserHelper sharedHelper].user.userID];
    if (ok) {       // 通知表情键盘
        [[WXUserHelper sharedHelper]updateEmojiGroupData];
    }
    return ok;
}
- (BOOL)deleteExpressionGroupByID:(NSString *)groupID{
    BOOL ok = [self.store deleteExpressionGroupByID:groupID forUid:[WXUserHelper sharedHelper].user.userID];
    if (ok) {       // 通知表情键盘
        [[WXUserHelper sharedHelper] updateEmojiGroupData];
    }
    return ok;
...
#pragma mark - 
- (WXDBExpressionStore *)store{
    if (_store == nil) {
        _store = [[WXDBExpressionStore alloc] init];
    }
    return _store;
}
- (NSMutableArray *)myExpressionListData{
    NSMutableArray *data = [[NSMutableArray alloc] init];
    NSMutableArray *myEmojiGroups = [NSMutableArray arrayWithArray:[self.store expressionGroupsByUid:[WXUserHelper sharedHelper].user.userID]];
    if (myEmojiGroups.count > 0) {
        WXSettingGroup *group1 = TLCreateSettingGroup(@"聊天面板中的表情", nil, myEmojiGroups);
        [data addObject:group1];
    }
    
    WXSettingItem *userEmojis = TLCreateSettingItem(@"添加的表情");
    WXSettingItem *buyedEmojis = TLCreateSettingItem(@"购买的表情");
    WXSettingGroup *group2 = TLCreateSettingGroup(nil, nil, (@[userEmojis, buyedEmojis]));
