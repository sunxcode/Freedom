...
@interface WXActionSheet() <UITableViewDataSource, UITableViewDelegate>{
...
- (instancetype)initWithTitle:(NSString *)title delegate:(id<WXActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ...{
    if (self = [super init]) {
...
- (void)showInView:(UIView *)view{
    ...
    CGRect rect = CGRectMake(0, self.frameHeight - self.actionSheetView.frameHeight, self.frame.size.width, self.actionSheetView.frameHeight);
...
- (void)p_initSubViews{
    ...
    if (self.cancelButtonTitle) {
        ...
        [self.cancelButton setFrame:CGRectMake(0, self.frameHeight - HEIGHT_BUTTON, self.frame.size.width, HEIGHT_BUTTON)];
...
- (void)dismissWithClickedButtonIndex:(NSInteger)buttonIndex animated:(BOOL)animated{
    ...
    if (animated) {
        CGRect rect = CGRectMake(0, self.frameHeight, self.frame.size.width, self.actionSheetView.frame.size.height);
        [UIView animateWithDuration:0.2 animations:^{
            [self.actionSheetView setFrame:rect];
            [self.backgroudView setBackgroundColor:[UIColor clearColor]];
