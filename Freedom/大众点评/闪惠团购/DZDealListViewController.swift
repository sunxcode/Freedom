...
class DZDealListViewVerticalCell:BaseTableViewCell{
    override func initUI() {
        ...
        let name = UILabel(frame: CGRect(x: XW(icon) + 10, y: 10, width: APPW - XW(icon) - 30, height: 20), font: fontSmallTitle, color: RGBAColor(0, 111, 255), text: nil)
...
class DZDealListViewController: DZBaseViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row % 5 != 0 {
            //竖着的
            cell = tableView.dequeueReusableCell(withIdentifier: DZDealListViewVerticalCell.getTableCellIdentifier()) as? BaseTableViewCell
            if cell == nil {
                cell = DZDealListViewVerticalCell.getInstance() as? BaseTableViewCell
            }
        } else {
            //横着的
            cell = tableView.dequeueReusableCell(withIdentifier: DZDealListViewTransverseCell.getTableCellIdentifier()) as? BaseTableViewCell
            if cell == nil {
