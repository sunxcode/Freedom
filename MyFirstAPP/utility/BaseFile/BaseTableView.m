...
- (void)loadData:(NSInteger)page{
    ...
    if ([[Utility Share] offline]&&self.bd_offline) {//离线状态
    ...
    if (self.bd_data) {//存在本地数据
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            // 耗时的操作
            NSArray *array = [[Utility Share] offline]?(self.bd_offline?self.bd_offline(_curPage):nil):self.bd_data(_curPage);
            dispatch_async(dispatch_get_main_queue(), ^{
                block(array,YES);
            });
        });
    }else if(self.urlString.length){//网络请求
        self.task = [NetEngine GET:self.urlString parameters:self.paramsDict progress:^(NSProgress * _Nonnull downloadProgress) {
...
//                self.backgroundView=nil;
                ...
                block([self.dataDic objectForJSONKey:@"list"],NO);
            }else{
                boolNull=YES;
