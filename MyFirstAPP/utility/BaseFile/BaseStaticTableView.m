...
@implementation SBaseItem
+ (instancetype)itemWithTitle:(NSString *)title{
    return [self itemWithTitle:title icon:nil];
}
+ (instancetype)itemWithTitle:(NSString *)title icon:(NSString *)icon{
    SBaseItem *item = [[self alloc] init];
    item.title = title;
    item.icon = icon;
    return item;
}
...
- (void)setIndexPath:(NSIndexPath *)indexPath rowsInSection:(int)rows{
    ...
    if (rows == 1) {
        bgView.image = [UIImage resizableImageWithName:@"white"];
        selectedBgView.image = [UIImage resizableImageWithName:@"gray"];
...
- (void)setItem:(SBaseItem *)item{
    ...
    if (item.badgeValue) { // 紧急情况：右边有提醒数字
        // 按钮的高度就是背景图片的高度
        self.bageView.frameHeight = self.bageView.currentBackgroundImage.size.height;
        // 设置文字
        [self.bageView setTitle:item.badgeValue forState:UIControlStateNormal];
        self.bageView.frameWidth  = self.bageView.currentBackgroundImage.size.width;
        self.accessoryView = self.bageView;
    } else if ([item isKindOfClass:[SBaseItem class]]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    // 1.取出这行对应的item模型
    SBaseGroup *group = self.groups[indexPath.section];
    SBaseItem *item = group.items[indexPath.row];
...
-(void)test{
    // 1.创建组
    SBaseGroup *group = [SBaseGroup group];
    [self.groups addObject:group];
    // 2.设置组的所有行数据
    SBaseItem *readMdoe = [SBaseItem itemWithTitle:@"阅读模式"];readMdoe.text = @"有图模式";
    SBaseItem *readMdoe1 = [SBaseItem itemWithTitle:@"字号大小"];readMdoe1.text = @"中";
    SBaseItem *readMdoe2 = [SBaseItem itemWithTitle:@"显示备注"];readMdoe2.text = @"是";
