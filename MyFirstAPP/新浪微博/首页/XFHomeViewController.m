...
-(void)setupUnreadCount {
    ...
    params[@"uid"] = account.uid;
    // 3.发送请求
    [NetEngine GET:@"https://rm.api.weibo.com/2/remind/unread_count.json" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        // NSNumber --> NSString
        // 设置提醒数字(微博的未读数)
...
-(void)refreshStatus:(UIRefreshControl *)control {
    ...
    [NetEngine GET:url parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
-(void)loadMoreStatus {
    ...
    // 3.发送请求
    [NetEngine GET:@"https://api.weibo.com/2/statuses/friends_timeline.json" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        // 将 "微博字典"数组 转为 "微博模型"数组
        NSArray *newStatuses = [XFStatus mj_objectArrayWithKeyValuesArray:responseObject[@"statuses"]];
